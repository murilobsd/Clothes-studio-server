...
def connect_mongo(mongo_settings, **kwargs):
    mongo_addr = kwargs.get('mongo_addr', {'host': mongo_settings['host'], 'port': mongo_settings['port']})
    mongo_db = kwargs.get('mongo_db', mongo_settings['db_name'])
    db = None
    for i in range(mongo_settings['reconnect_tries'] + 1):
        try:
            ...
            if i >= mongo_settings['reconnect_tries']:
                ...
                l.warning("ConnectionFailure #{0} during server start, ""waiting {1} seconds".format(i+1, timeout))
                time.sleep(timeout)
        else:
            break
    return db